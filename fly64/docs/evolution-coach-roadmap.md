...
- **CSC 优化**（P1-2）：**负结果留档（2026-09-16）**——选择路径微优化（布尔掩码/fancy-index/gather+bincount 三变体）实测全部落在负载噪声内（交错中位数 0.95–1.38×），成本本质是选中 nnz 的内存带宽。剩余方向：稠密核心拆分（高度数神经元稠密块 + BLAS）、GPU 批处理、或降边数。**不要重试选择路径微调**
