...
- **CSC 优化**（P1-2）：**负结果留档（2026-09-16）**——选择路径微优化（布尔掩码/fancy-index/gather+bincount 三变体）实测全部落在负载噪声内（交错中位数 0.95–1.38×），成本本质是选中 nnz 的内存带宽。剩余方向：稠密核心拆分（高度数神经元稠密块 + BLAS）、GPU 批处理、或降边数。**不要重试选择路径微调**。二轮基准（2026-09-16）：全量 SpMV（`w @ 0/1向量`）实测同样更慢——5% 放电时选择法 3.9ms vs CSC matvec 24.0ms / CSRᵀ matvec 20.2ms；且 matvec 改变浮点累加顺序，会破坏 replay.py 逐 tick 精确断言（allclose 1e-4 即失败）。**结论：选择法在 5–15% 放电区间已是 scipy 格式下的地板（8–15ms）。**下一级优化只剩 GPU（状态驻留显存需架构决策）或稠密核心拆分（BLAS gemv，n_hubs×n 密集块受显存/带宽限制）。务实建议：接受当前性能，把 GPU 预算投向**反事实批处理**（N 组参数并行重仿真）——直接加速进化验证吞吐，风险远低于主循环移植。
