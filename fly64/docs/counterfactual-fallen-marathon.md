...
## 第二轮实验（转向压制源分解，EVO-046）

首轮把压制归因于 fallen forward 振幅——**归因修正**：0.20 与 0.50 振幅输出完全一致（|x| 同为 0.202，LIF 饱和），真正引入压制的是 `escape_jump_drive=True` 本身。分解实验（同分块）：

| 场景 | mean_abs_x | mean_y |
|------|-----------:|-------:|
| baseline（escape off） | 4.194 | 62.44 |
| escape on（jump 0.45 + fwd 0.20） | 0.392 | 61.01 |
| escape on + 跳跃爆发**关闭** | 0.676 | 63.12 |
| escape on + 跳跃爆发**减半**（0.225） | 1.104 | 62.65 |

**结论：转向压制是多源叠加、剂量依赖**——跳跃爆发贡献一部分（剂量依赖可部分恢复），剩余来自 fallen forward 与 escape_mode 电流。单一旋钮无法恢复转向。

**修正后的修复方向**：不是调某一个电流的振幅，而是**对 fallen 期间的全部逃逸电流族做统一占空比**（time-share 整族），或按 P1-2 负结果的经验直接跳过微调类方案、走结构化分时调度。
