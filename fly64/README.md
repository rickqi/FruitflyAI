...
实际运行中暴露的典型场景（截图记录）：马里奥面对锁门，画面出现对话文字 *"You need a key to open this door"*，此时：
...
**结论**：因果可视化解决的是"可解释性"，而非"能力"。语义级场景理解（文字/物品识别）、道具获取（定位钥匙→路径规划→拾取→返回开门）属于更高层认知能力，需要引入视觉语言模型或目标条件规划，见下方路线图远期方向。
...
### Phase 4（远期）: 语义理解与道具任务

解决上述"需要钥匙"类语义场景的根本方案（需外部模型能力，非果蝇脑模型本身可达）：

| 任务 | 说明 |
|:----|:------|
| **P4.1** 对话文字识别 | 对话框区域裁剪 → OCR/VLM 提取任务语义（"need key"→目标：钥匙） |
| **P4.2** 目标条件记忆 | SpatialMemoryMap 扩展：记录钥匙等道具出现位置，形成"取物→返回"任务图 |
| **P4.3** VLM 场景问答 | 关键帧送视觉语言模型，回答"现在该做什么"并映射为子目标 |
| **P4.4** 任务级行动原语 | 拾取/开门/使用道具的动作序列，注入控制级联为新优先级层 |
