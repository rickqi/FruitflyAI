...
for f in fly64/main.py fly64/memory.py skills/skills.md skills/evolution_history.json \
         tests/test_fall_state_accuracy.py tests/test_memory.py scripts/ver_append_fix4.py; do
  cp "$DEV/$f" "$DEP/$f"
done
...
./venv/bin/python -m pytest tests/test_fall_state_accuracy.py tests/test_memory.py -q 2>&1 | tail -1
python3 - <<'EOF'
