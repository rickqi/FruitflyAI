...
> 当前版本：BRAIN_VERSION **2.18.0** / SKILL_VERSION **3.0.0**（R17 MBON 稳态缩放 → R18 DAN 塑形 → R19 躁动电流 → R20 CX 罗盘自主+目标向量 → t20 咨询阈值 60s → t21 读屏+帧留存 → R21 coach strategy visibility → 2.14.0 桥接 Z 触发解锁 → 2.15.0 CPG 运动原语级联层 → 2.16.0 strike/crouch 神经池 + MBON 9 列 → 2.17.0 债务清理（fix_catalog 修复 + 直写 KPI 预算化）→ 2.18.0 M2：教官原语策略 + WALL/SLIDING 状态 + 踢墙跳/侧空翻。完整逐轮记录见 **skills/evolution_history.json**）。
