...
| Metric | Value |
|--------|-------|
| Dopamine Gain Avg | 0.6344 |
| Learning Progress (mean |error| over 100 ticks) | 0.4055 |
| Mushroom Weight Changes (assoc_count) | 0 |
| Reward Trend (cumulative) | -1.7959 |
| Error Gradient Mean | 0.4055 |
| Gain Update Count | 85162 |

## Pattern Catalog

The following 13 patterns are loaded from `default_patterns.json` and validated against JSON Schema (draft-07).
...
**Time**: 2026-09-15 14:40:22 UTC

**Findings**: 5 pattern(s)
- [HIGH] Below ground stuck — Y坐标异常导致无法移动 (conf=100%)
- [HIGH] Suspended animation — 控制信号为零停止响应 (conf=100%)
- [HIGH] Micro-loop weave — 原地编织（交替未产生位移） (conf=100%)
- [HIGH] Micro-loop weave (signal-only) — 原地编织（行为信号判定，绕过 anomaly 分类器） (conf=100%)
- [MEDIUM] Cliff standoff — 悬崖边缘对峙驻留 (conf=100%)


---

*Auto-generated by EvolutionSkill v3.0.0 on 2026-09-15 14:40:22 UTC*
